---
文档名-title: HappyLLm课程笔记章节02.1 - 注意力机制
创建时间-create time: 2025-07-20 21:39
更新时间-modefived time: 2025-07-20 21:39 星期日
文档粗分-text: 笔记
笔记细分-text: 
笔记索引-link: '[[笔记总索引]]'
继承自-link: 
...
[![图片描述](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/1-0.png)](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/1-0.png)

图2.1 前馈神经网络

- 卷积神经网络（Convolutional Neural Network，CNN），即训练参数量远小于前馈神经网络的卷积层来进行特征提取和学习，如图2.2所示:

[![图片描述](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/1-1.png)](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/1-1.png)

图2.2 卷积神经网络

- 循环神经网络（Recurrent Neural Network，RNN），能够使用历史信息作为输入、包含环和自重复的网络，如图2.3所示:

[![图片描述](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/1-2.png)](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/1-2.png)
...
![[Pasted image 20250720214758.png]]

# 核心：如何针对每一个 Query，计算出对应的注意力分数呢？核心公式
...
那么，我们就可以用点积来计算词之间的相似度。假设我们的 Query 为“fruit”，对应的词向量为 q ；我们的 Key 对应的词向量为 k=[vapplevbananavchair] ,则我们可以计算 Query 和每一个键的相似程度：
...
> 想象一下，我们有一堆拼图碎片，每一块碎片都有自己的特征。注意力机制就像是一个“拼图匹配器”，它的任务是找出每一块碎片和其他碎片之间的相似程度，然后根据相似度来决定每一块碎片的重要性，也就是“分配注意力”。
> 
> 在这个过程中，我们有三组碎片：Q（查询碎片）、K（关键碎片）和V（价值碎片）。通常情况下，Q来自一个序列，而K和V来自另一个序列。比如在Transformer的Decoder部分，Q来自当前正在处理的输入，而K和V来自之前编码好的信息。这个过程就像是用Q去“询问”K，看看它们之间的相似度，然后根据相似度从V中提取有用的信息。
> 
> 但在Transformer的Encoder部分，我们用的是自注意力机制，这就好像是在一个封闭的房间里，所有的碎片都来自同一个序列。Q、K、V都是从同一个输入序列通过不同的“过滤器”（参数矩阵）得到的。就像是让每一块碎片都去和其他碎片比较，看看它们之间的相似度，然后根据这些相似度来决定每一块碎片的重要性。
> 
> 通过自注意力机制，我们可以找出每一块碎片和其他所有碎片之间的关系，从而更好地理解整个拼图的结构。在代码实现中，这个过程就像是把同一个输入通过不同的“过滤器”来得到Q、K、V，从而实现自注意力。


### 2.1.5 掩码自注意力

[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#215-%E6%8E%A9%E7%A0%81%E8%87%AA%E6%B3%A8%E6%84%8F%E5%8A%9B)
...
[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#216-%E5%A4%9A%E5%A4%B4%E6%B3%A8%E6%84%8F%E5%8A%9B)
...
[![图片描述](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/1-3.jpeg)](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/1-3.jpeg)
...
[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#22-encoder-decoder)
...
[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#221-seq2seq-%E6%A8%A1%E5%9E%8B)
...
[![图片描述](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/2-0.jpg)](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/2-0.jpg)
...
[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#222-%E5%89%8D%E9%A6%88%E7%A5%9E%E7%BB%8F%E7%BD%91%E7%BB%9C)
...
[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#223-%E5%B1%82%E5%BD%92%E4%B8%80%E5%8C%96)
...
[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#224-%E6%AE%8B%E5%B7%AE%E8%BF%9E%E6%8E%A5)
...
[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#225-encoder)
...
[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#226-decoder)
...
[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#23-%E6%90%AD%E5%BB%BA%E4%B8%80%E4%B8%AA-transformer)

在前两章，我们分别深入剖析了 Attention 机制和 Transformer 的核心——Encoder、Decoder 结构，接下来，我们就可以基于上一章实现的组件，搭建起一个完整的 Transformer 模型。

### 2.3.1 Embedding 层

[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#231-embedding-%E5%B1%82)
...
[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#232-%E4%BD%8D%E7%BD%AE%E7%BC%96%E7%A0%81)
...
xPE=[0.10.20.30.4 0.20.30.40.5 0.30.40.50.6 0.40.50.60.7]+[sin⁡(0100000)cos⁡(0100000)sin⁡(0100002/4)cos⁡(0100002/4) sin⁡(1100000)cos⁡(1100000)sin⁡(1100002/4)cos⁡(1100002/4) sin⁡(2100000)cos⁡(2100000)sin⁡(2100002/4)cos⁡(2100002/4) sin⁡(3100000)cos⁡(3100000)sin⁡(3100002/4)cos⁡(3100002/4)]=[0.11.20.31.4 1.0410.840.411.49 1.209−0.0160.521.59 0.541−0.4890.8951.655]
...
这里加上的 pm， pn 就是位置编码。接下来我们将 f(...,xm+pm,...,xn+pn) 在 m,n 两个位置上做泰勒展开：
...
可以看到第1项与位置无关，2～5项仅依赖单一位置，第6项（f 分别对 m、n 求偏导）与两个位置有关，所以我们希望第六项（ pmTHpn ）表达相对位置信息，即求一个函数 g 使得:

pmTHpn=g(m−n)

我们假设 H 是一个单位矩阵，则：

pmTHpn=pmTpn=⟨pm,pn⟩=g(m−n)
...
[![图片描述](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/3-0.png)](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/3-0.png)
...
[](https://github.com/datawhalechina/happy-llm/blob/main/docs/chapter2/%E7%AC%AC%E4%BA%8C%E7%AB%A0%20Transformer%E6%9E%B6%E6%9E%84.md#233-%E4%B8%80%E4%B8%AA%E5%AE%8C%E6%95%B4%E7%9A%84-transformer)

上述所有组件，再按照下图的 Tranfromer 结构拼接起来就是一个完整的 Transformer 模型了，如图2.7所示：

[![图片描述](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/3-1.png)](https://raw.githubusercontent.com/datawhalechina/happy-llm/main/docs/images/2-figures/3-1.png)
...
[1] Ashish Vaswani, Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N. Gomez, Lukasz Kaiser, Illia Polosukhin. (2023). _Attention Is All You Need._ arXiv preprint arXiv:1706.03762.

[2] Jay Mody 的文章 “An Intuition for Attention”. 来源：[https://jaykmody.com/blog/attention-intuition/](https://jaykmody.com/blog/attention-intuition/)